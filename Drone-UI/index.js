...
const loadManModel = async (position, index) => {
  try {
    ...
    console.log(
      `✅ Man model loaded at index ${index} with red color and correct scaling.`,
    );
  } catch (error) {
...
const findClosestMen = () => {
  ...
  closestMenPositions = menPositions
    ...
    .map((pos) => ({
      ...pos,
      distance: Math.sqrt(
        Math.pow(pos.x - charPos.x, 2) + Math.pow(pos.z - charPos.z, 2),
      ),
    }))
...
const isPositionInActiveTerrain = (position) => {
  for (const tile of terrainTiles) {
    ...
    const xInRange =
      position.x >= tileCoords.x && position.x <= tileCoords.x + tileWidth;
    const zInRange =
      position.z >= tileCoords.y && position.z <= tileCoords.y + tileWidth;
...
const keyDown = (event) => {
  ...
  if (event.keyCode === 71 && !isNavigating) {
    // 'G' key
    if (menNeedingHeightUpdate.length === 0) {
      // Only allow if all heights are set
...
const determineMovement = () => {
  ...
  if (activeKeysPressed.includes(87)) {
    // W key
    character.translateZ(0.4);
  }
  if (activeKeysPressed.includes(83)) {
    // S key
    character.translateZ(-0.4);
  }
  ...
  if (activeKeysPressed.includes(40)) {
    // Down arrow
...
const generateFixedTargets = () => {
  ...
  allTargets = closestMenPositions.map(
...
const moveToNextTarget = () => {
  ...
  character.rotation.y +=
    (targetRotation - character.rotation.y) * rotationSpeed;
  ...
  if (distance < 3) {
    ...
    if (lastSentIndex !== currentTargetIndex) {
      ...
      console.log(
        `📍 Reached Target ${currentTargetIndex + 1}: ${message}, ${modelType}`,
      );
