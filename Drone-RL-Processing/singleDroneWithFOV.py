...
class CustomFeatureExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: spaces.Dict):
        ...
        self.position_net = nn.Sequential(
            nn.Linear(3, 32), nn.ReLU(), nn.Linear(32, 64)
        )

        self.imu_net = nn.Sequential(nn.Linear(7, 32), nn.ReLU(), nn.Linear(32, 64))
        ...
        self.combine_net = nn.Sequential(nn.Linear(64 + 64 + 128, 256), nn.ReLU())
...
class SearchDroneEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.drone = p.loadURDF(
            "./data/Quadrotor/quadrotor.urdf", start_pos, useFixedBase=False
        )
        ...
        for _ in range(num_victims):
            ...
            collision_shape_id = p.createCollisionShape(
                shapeType=p.GEOM_SPHERE, radius=victim_sphere_radius
            )
            ...
            self.victims.append(
                {"pos": [x / 5, y / 5, 0], "found": False, "body_id": victim_id}
            )
...
def train():
    ...
    checkpoint = CheckpointCallback(
        save_freq=100_000, save_path=config["save_dir"], name_prefix="search_drone"
    )
