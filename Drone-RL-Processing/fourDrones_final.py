...
class MissionMonitor:
    ...
    def print_mission_status(self):
        ...
        coverage_percentage = (
            np.sum(self.coverage_map > 0) / self.coverage_map.size
        ) * 100
        ...
        for i, human in enumerate(self.humans_found, 1):
            print(
                f"  {i}. Position: ({human['position'][0]:.1f}, {human['position'][1]:.1f})"
            )
        ...
        if self.humans_found:
            ...
            for i, target in enumerate(route, 1):
                print(
                    f"  {i}. Go to ({target['position'][0]:.1f}, {target['position'][1]:.1f})"
                )
...
class EmailMissionMonitor(MissionMonitor):
    ...
    def _calculate_mission_stats(self):
        ...
        search_speed = (
            coverage_percentage / (elapsed_time / 60) if elapsed_time > 0 else 0
        )

        # Calculate search patterns
        visited_areas = np.count_nonzero(self.visitation_heatmap)
        revisit_ratio = (
            (np.sum(self.visitation_heatmap > 1) / visited_areas)
            if visited_areas > 0
            else 0
        )
        ...
        return {
            ...
            "covered_area": (coverage_percentage / 100)
            * abs(
                (self.search_area[1] - self.search_area[0])
                * (self.search_area[3] - self.search_area[2])
            ),
    ...
    def send_status_email(self, force=False):
        ...
        msg["Subject"] = (
            f"🚨 Search and Rescue Status - {len(self.humans_found)} Located"
        )
...
class CustomFeatureExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: spaces.Dict):
        ...
        self.imu_net = nn.Sequential(nn.Linear(7, 64), nn.ReLU(), nn.Linear(64, 128))
        ...
        self.combine_net = nn.Sequential(nn.Linear(128 + 128 + 256, 512), nn.ReLU())
    ...
    def forward(self, observations) -> torch.Tensor:
        ...
        combined = torch.cat(
            [position_features, imu_features, coverage_features], dim=1
        )
...
class MultiDroneEnv(gym.Env):
    def __init__(self, num_drones=4, render=True):
        ...
        self.monitor = EmailMissionMonitor(
            self.search_area, self.grid_size, email_config
        )
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "coverage_map": spaces.Box(
                    low=0,
                    high=1,
                    shape=(self._get_grid_size(), self._get_grid_size()),
                    dtype=np.float32,
                ),
                "visitation_map": spaces.Box(
                    low=0,
                    high=np.inf,
                    shape=(self._get_grid_size(), self._get_grid_size()),
                    dtype=np.float32,
    ...
    def _get_grid_size(self):
        ...
        if grid_width <= 0 or grid_height <= 0:
            raise ValueError(
                f"Invalid grid dimensions: {grid_width}x{grid_height}. Check search_area and grid_size parameters."
            )
    ...
    def reset(self):
        ...
        for i in range(self.num_drones):
            ...
            start_pos = [
                np.clip(
                    (area[0] + area[1]) / 2, self.search_area[0], self.search_area[1]
                ),
                np.clip(
                    (area[2] + area[3]) / 2, self.search_area[2], self.search_area[3]
                ),
                self.fixed_height,
            ]
            ...
            try:
                drone = p.loadURDF(
                    "./data/Quadrotor/quadrotor.urdf", start_pos, globalScaling=2.0
                )
        ...
        for i in range(num_victims):
            ...
            try:
                visual_shape = p.createVisualShape(
                    shapeType=p.GEOM_SPHERE, radius=0.5, rgbaColor=[1, 0, 0, 0.7]
                )
                collision_shape = p.createCollisionShape(
                    shapeType=p.GEOM_SPHERE, radius=0.5
                )
                ...
                self.victims.append(
                    {"pos": victim_pos, "found": False, "body_id": victim_id}
                )
            except Exception as e:
    ...
    def step(self, actions):
        ...
        drone_positions = [
            p.getBasePositionAndOrientation(drone)[0] for drone in self.drones
        ]
        ...
        for victim in self.victims:
            if not victim["found"]:
                ...
                if distance < self.fov_radius:
                    ...
                    p.changeVisualShape(victim["body_id"], -1, rgbaColor=[0, 1, 0, 0.7])
        return detected
    ...
    def _calculate_reward(self):
        ...
        for pos in drone_positions:
            # Boundary penalty
            if (
                pos[0] < self.search_area[0]
                or pos[0] > self.search_area[1]
                or pos[1] < self.search_area[2]
                or pos[1] > self.search_area[3]
            ):
                total_reward -= 5.0
...
def train_multi_drone():
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=100000, save_path="./drone_models", name_prefix="multi_drone"
    )
